...
def apply_home_prime(galaxy_data, defaults, capitals):
    """Prime restored capital systems and homeworld planets (Woh Prime / Weeer Prime)."""
    ...
    for c in defaults:
        ...
        if not sys:
            continue
        sys_base = _plain_name(sys.get("name") or "")
        ...
        if sys_base:
            sys["name"] = _with_prime(sys.get("name") or sys_base)
        if not hw:
            continue
        root = sys.get("hierarchy_root")
        queue = [root] if root else []
...
GESTALT_SKIP_TECHS = frozenset({
    ...
    "tech_gene_tailoring", "tech_vitality_boosters", "tech_living_state",
    "tech_collective_self", "tech_epigenetic_triggers", "tech_xeno_relations",
    "tech_slave_colonies", "tech_penal_colonies", "tech_resort_colonies",
    "tech_effective_bureaucracy", "tech_interstellar_economics", "tech_psionic_theory",
    "tech_alien_cloning", "tech_improved_incubators", "tech_metabolic_gases",
...
_PREF_TRAITS = (
    ...
    "trait_pc_arid_preference", "trait_pc_desert_preference", "trait_pc_tundra_preference",
    "trait_pc_arctic_preference", "trait_pc_alpine_preference", "trait_pc_savannah_preference",
    "trait_pc_nuked_preference", "trait_pc_gaia_preference",
)


def legal_successor_gov(emp, sp, rng, civic_keys=None, ethic_keys=None):
    """Authority + civics that Stellaris will actually accept for these ethics."""
    if is_machine(emp, sp):
        return (
            "auth_machine_intelligence",
            ["civic_machine_builder", "civic_machine_replication"],
            ["ethic_gestalt_consciousness"],
        )
    if is_hive(emp, sp):
        return (
            "auth_hive_mind",
            ["civic_hive_divided_attention", "civic_hive_one_mind"],
            ["ethic_gestalt_consciousness"],
        )
    ethics = [
        e for e in (emp.get("ethics") or [])
        if e != "ethic_gestalt_consciousness" and "gestalt" not in str(e)
    ]
    if ethic_keys:
        ethics = [e for e in ethics if e in ethic_keys]
    if not ethics:
        ethics = ["ethic_xenophile", "ethic_materialist"]
    ethics = ethics[:3]
    s = set(ethics)
    fan_eg = "ethic_fanatic_egalitarian" in s
    eg = fan_eg or "ethic_egalitarian" in s
    fan_au = "ethic_fanatic_authoritarian" in s
    au = fan_au or "ethic_authoritarian" in s
    if fan_eg:
        authority = "auth_democratic"
    elif fan_au:
        authority = rng.choice(["auth_dictatorial", "auth_imperial"])
    elif eg and not au:
        authority = rng.choice(["auth_democratic", "auth_oligarchic"])
    elif au and not eg:
        authority = rng.choice(["auth_oligarchic", "auth_dictatorial", "auth_imperial"])
    else:
        authority = "auth_oligarchic"
    pool = [
        "civic_mining_guilds", "civic_functional_architecture",
        "civic_efficient_bureaucracy", "civic_environmentalist",
    ]
    if not fan_eg:
        pool.append("civic_police_state")
    # Meritocracy is democratic/oligarchic only (vanilla possible = dem|oli).
    if authority in ("auth_democratic", "auth_oligarchic"):
        pool.append("civic_meritocracy")
    if civic_keys:
        pool = [c for c in pool if c in civic_keys] or [
            "civic_mining_guilds", "civic_functional_architecture",
        ]
    if len(pool) >= 2:
        civics = rng.sample(pool, 2)
    else:
        civics = (pool + ["civic_mining_guilds", "civic_functional_architecture"])[:2]
    return authority, civics, ethics


_ETHIC_FLIP = {
    "ethic_xenophobe": "ethic_xenophile",
    "ethic_fanatic_xenophobe": "ethic_fanatic_xenophile",
    "ethic_xenophile": "ethic_xenophobe",
    "ethic_fanatic_xenophile": "ethic_fanatic_xenophobe",
    "ethic_militarist": "ethic_pacifist",
    "ethic_fanatic_militarist": "ethic_fanatic_pacifist",
    "ethic_pacifist": "ethic_militarist",
    "ethic_fanatic_pacifist": "ethic_fanatic_militarist",
    "ethic_authoritarian": "ethic_egalitarian",
    "ethic_fanatic_authoritarian": "ethic_fanatic_egalitarian",
    "ethic_egalitarian": "ethic_authoritarian",
    "ethic_fanatic_egalitarian": "ethic_fanatic_authoritarian",
    "ethic_materialist": "ethic_spiritualist",
    "ethic_fanatic_materialist": "ethic_fanatic_spiritualist",
    "ethic_spiritualist": "ethic_materialist",
    "ethic_fanatic_spiritualist": "ethic_fanatic_materialist",
}


def buffer_successor_gov(emp, sp, rng, civic_keys=None, ethic_keys=None):
    """Fringe breakaways: inverted ethics so they rival the parent."""
    if is_machine(emp, sp):
    ...
    flipped["ethics"] = ethics
    return legal_successor_gov(flipped, sp, rng, civic_keys, ethic_keys)


def clamp_default_starbases(starbases):
    """Default successors cannot use FE citadel designs or starhold without tech."""
    out = {}
    for sid, sz in (starbases or {}).items():
        s = str(sz)
        if s in ("starbase_outpost", "starbase_starport"):
            out[str(sid)] = s
        else:
            out[str(sid)] = "starbase_starport"
    return out


def _place_label(place):
    s = display_empire_name(place or "")
    s = re.sub(r"\s+(Black Hole|Prime|A|B|C)$", "", s, flags=re.I).strip()
    if continuum_cp._is_junk_label(s) or s in ("Empire", "Unknown", "Star", ""):
        return ""
    return s


def unique_successor_name(sp_name, authority, used, rng, place=None, sibling_index=0):
    """First sibling: species + gov noun. Later: landmark/system name + noun."""
    ...
    rng.shuffle(nouns)
    loc = _place_label(place)
    generics = ["Rim", "March", "Reach", "Veil", "Expanse", "Drift", "Watch", "Hold", "Cloister"]
    if sibling_index == 0:
        for noun in nouns:
            ...
            if nm and not continuum_cp._is_junk_label(nm) and nm not in used:
                used.add(nm)
                return nm
    label = loc or rng.choice(generics)
    for noun in nouns:
        nm = f"{label} {noun}"
        if nm not in used:
            used.add(nm)
    ...
    for g in generics:
        for noun in nouns:
            ...
            if nm not in used:
                ...
                return nm
    i = 2
    nm = f"{label} {nouns[0] if nouns else 'Polity'}"
    while nm in used:
        i += 1
        nm = f"{label} {nouns[0] if nouns else 'Polity'} {i}"
    used.add(nm)
    return nm
...
def _void_trait_lines(traits, trait_keys=None):
    ...
    for t in traits or []:
        ...
        kept.append(t)
    kept = kept[:7]
    kept.insert(0, "trait_pc_habitat_preference")
    return "\n".join(f"\t\t\t\t\ttrait = {t}" for t in kept)
...
"""
        if cid in skip_aged:
            ...
            rem_chunk = (
                "			if = {\n"
                "				limit = { NOT = { has_global_flag = continuum_aged } }\n"
                + inner
                + "\n			}\n"
            )
...
""")
    ...
    _FLAG_COLORS = [
        "red", "blue", "teal", "orange", "black", "white", "dark_grey", "burgundy",
        "turquoise", "purple", "pink", "brown", "yellow", "green", "dark_purple",
        "dark_blue", "dark_green", "grey", "beige",
    ]
    _FLAG_BG = [
        "00_solid.dds", "new_dawn.dds", "circle.dds", "stripes.dds", "v.dds",
        "inverted_v.dds", "horizontal.dds", "flag_BG_1.dds", "flag_BG_5.dds",
        "diagonal.dds", "sinus.dds", "triangle_split.dds", "vertical.dds",
    ]
    _FLAG_ICONS = [
        ("spherical", "flag_spherical_1.dds"), ("spherical", "flag_spherical_7.dds"),
        ("pointy", "flag_pointy_1.dds"), ("pointy", "flag_pointy_8.dds"),
        ("blocky", "flag_blocky_1.dds"), ("blocky", "flag_blocky_9.dds"),
        ("ornate", "flag_ornate_2.dds"), ("pirate", "flag_pirate_4.dds"),
        ("human", "flag_human_3.dds"), ("domination", "domination_5.dds"),
        ("corporate", "corporate_03.dds"), ("imperial", "imperial_04.dds"),
        ("zoological", "flag_zoological_4.dds"), ("special", "pirate_flag.dds"),
    ]
    used_names = set()
    parent_sib = {}
    for emp0 in empires:
        nm = display_empire_name(emp0.get("name") or "")
        if nm:
            used_names.add(nm)
    for sidx, emp in enumerate(plan.get("aged_splinters") or []):
        ...
        if emp.get("_buffer"):
            authority, civics, ethics = buffer_successor_gov(emp, sp, _spl_rng, civic_keys, ethic_keys)
        else:
            authority, civics, ethics = legal_successor_gov(emp, sp, _spl_rng, civic_keys, ethic_keys)
        pid = emp.get("_parent_id")
        sib = parent_sib.get(pid, 0)
        parent_sib[pid] = sib + 1
        place = emp.get("_home")
        if not place:
        ...
        if is_void:
            ...
            ideal = "pc_habitat"
            void_tech = ""
            if not tech_keys or "tech_habitat" in tech_keys:
                void_tech = "\n						give_technology = { tech = tech_habitat message = no }"
        else:
            trait_lines = "\n".join(f"\t\t\t\t\ttrait = {t}" for t in traits[:8]) or "\t\t\t\t\ttrait = trait_adaptive"
            ideal = "pc_continental"
            void_tech = ""
        colors = list(emp.get("flag_colors") or ["orange", "black", "black", "null"])
        while len(colors) < 4:
            colors.append("null")
        keep = colors[0]
        others = [c for c in _FLAG_COLORS if c != keep]
        if sib == 0:
            colors[1] = _spl_rng.choice(others)
            colors[2] = _spl_rng.choice([c for c in others if c != colors[1]] or others)
        else:
            colors[0] = _spl_rng.choice(_FLAG_COLORS)
            colors[1] = _spl_rng.choice([c for c in _FLAG_COLORS if c != colors[0]])
            colors[2] = _spl_rng.choice([c for c in _FLAG_COLORS if c not in (colors[0], colors[1])] or others)
        gfx = emp.get("graphical_culture") or "mammalian_01"
        emp["starbases"] = clamp_default_starbases(emp.get("starbases") or {})
        tech_extra = []
        for sz in (emp.get("starbases") or {}).values():
            for t in STARBASE_TECHS.get(sz, ("tech_starbase_1", "tech_starbase_2")):
                if t not in tech_extra and (not tech_keys or t in tech_keys):
                    tech_extra.append(t)
        sb_tech = "".join(
            f"\n						give_technology = {{ tech = {t} message = no }}" for t in tech_extra
        )
        sb_tech += "\n						refresh_auto_generated_ship_designs = yes"
        icon_cat = emp.get("flag_icon_cat") or "special"
        icon_file = emp.get("flag_icon") or "pirate_flag.dds"
        if sib >= 1:
            icon_cat, icon_file = _spl_rng.choice(_FLAG_ICONS)
        bg_file = _spl_rng.choice(_FLAG_BG)
        restored.append({
            "emp": emp,
            "prefix": "continuum_spl",
            "idx": sidx,
            "owner": f"continuum_spl_{sidx}",
            "cap_flag": f"continuum_spl_{sidx}_capital",
            "_sp_class": sp.get("class") or "",
            "pop_txt": _pop_blocks(emp, "continuum_spl", sidx, f"continuum_spl_{sidx}", f"continuum_spl_sp_{sidx}"),
            "sb_txt": _starbase_blocks("continuum_spl", sidx, f"continuum_spl_{sidx}", emp),
        })
        chunks.append(f"""			if = {{
				limit = {{ has_global_flag = continuum_aged }}
				create_species = {{
					...
					namelist = {sp.get('namelist', 'MAM1')}
					traits = {{
						ideal_planet_class = {ideal}
{trait_lines}
					}}
				}}
				last_created_species = {{
					save_global_event_target_as = continuum_spl_sp_{sidx}
				}}
				create_country = {{
					name = {script_name(country_name)}
					type = default
					authority = {authority}
					civics = {{ civic = {civics[0]} civic = {civics[1]} }}
					origin = {origin}
					...
					ethos = {{ {' '.join(f'ethic = {e}' for e in ethics[:3])} }}
					flag = {{
						icon = {{ category = "{icon_cat}" file = "{icon_file}" }}
						background = {{ category = "backgrounds" file = "{bg_file}" }}
						colors = {{ "{colors[0]}" "{colors[1]}" "{colors[2]}" "{colors[3]}" }}
					}}
					ignore_initial_colony_error = yes
					day_zero_contact = no
					exclude_day_zero_contact = event_target:continuum_human
					effect = {{
						save_global_event_target_as = continuum_spl_{sidx}
						set_graphical_culture = {gfx}
						set_country_flag = continuum_aged_splinter{void_tech}{sb_tech}
						add_resource = {{ energy = 800 minerals = 800 food = 800 alloys = 300 influence = 150 }}
					}}
				}}
			}}
""")
...
""")
    ...
    for idx, prim in enumerate(plan.get("primitives") or []):
        ...
        if is_hive(prim, sp):
            prim_auth = "auth_hive_mind"
            ethics = ["ethic_gestalt_consciousness"]
            civics = ["civic_hive_divided_attention", "civic_hive_one_mind"]
        elif is_machine(prim, sp):
            prim_auth = "auth_machine_intelligence"
            ethics = ["ethic_gestalt_consciousness"]
            civics = ["civic_machine_builder", "civic_machine_replication"]
...
{trait_lines}
				...
				if = {{
					...
					create_country = {{
						...
						origin = origin_default
						species = last_created_species
						ethos = {{ {' '.join(f'ethic = {e}' for e in ftl_ethics[:3])} }}
						flag = {{
							icon = {{ category = "{prim.get('flag_icon_cat', 'special')}" file = "{prim.get('flag_icon', 'pirate_flag.dds')}" }}
						...
						effect = {{
							set_graphical_culture = {gfx}
							set_country_flag = continuum_pre_primitive_ftl
							save_global_event_target_as = continuum_prim_{idx}
							add_resource = {{ energy = 600 minerals = 600 food = 600 alloys = 200 influence = 100 }}
						}}
					}}
					solar_system = {{
						create_starbase = {{
...
""")
    ...
    for nidx, np in enumerate(plan.get("aged_new_prims") or []):
        cls = np.get("class") or "MAM"
        age = np.get("age") or "iron_age"
        nlist = np.get("namelist") or "MAM1"
        pc = np.get("planet_class") or "pc_continental"
        chunks.append(f"""			if = {{
				limit = {{ has_global_flag = continuum_aged }}
				every_system = {{
					limit = {{ has_star_flag = continuum_newprim_{nidx} }}
					every_system_planet = {{
						limit = {{
							has_planet_flag = continuum_newprim_{nidx}_homeworld
							is_colony = no
							NOT = {{ exists = owner }}
						}}
						create_species = {{
						...
						create_country = {{
							...
							effect = {{
								set_country_flag = {age}
								set_pre_ftl_age = {age}
								set_country_flag = continuum_aged_new_primitive
								save_global_event_target_as = continuum_newprim_{nidx}
							}}
						}}
						create_colony = {{
							owner = last_created_country
...
""")
    chunks.append("""			if = {
				limit = { has_global_flag = continuum_aged }
				every_system = {
					limit = { has_star_flag = continuum_aged_fallow }
					every_system_planet = {
						limit = { is_star = no }
						set_planet_flag = continuum_aged_ruins
					}
				}
			}
""")
...
def build_empire_plan(save_path, galaxy_data, stars, planets, argv_start=None):
    ...
    return {
        ...
        "countries": countries,
        "devastation_system": None,
        "planet_to_system": p2s,
        "colony_to_planet": colony_to_planet,
    }
